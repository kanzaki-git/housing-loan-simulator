...
// Leaflet の世界地図は横方向に繰り返せるため、
// 保存する経度は必ず -180〜180 度に正規化する。
...
function hasValidMapPosition(area) {
  return (
    area &&
    area.lat !== null &&
    area.lat !== undefined &&
    ...
    normalizeLongitude(area.lng) !== null
  );
}
...
function loadLandAreas() {
  try {
    ...
    landAreas.forEach((area) => {
      ...
      if (Number(area.lng) !== normalizedLng) {
        area.lng = normalizedLng;
        normalized = true;
      }
    });
...
function updateCandidateMap(
  ...
  // 折り畳み内で初期化された場合でも、
  // 現在の表示サイズを先に Leaflet へ伝える。
  candidateMap.invalidateSize(false);
